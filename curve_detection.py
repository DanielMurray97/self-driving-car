...
def get_lane_curve(img, display=2):
    # Where you enter the Morph Values
    initial_trackbar_vals = [110, 395, 90, 451]
    # Sends the Morph Values to Background to initialize
    bck.initialize_trackbars(initial_trackbar_vals)

    img_copy = img.copy()
    img_result = img.copy()
    # Lane detection (Seperates lane from rest of image)
    img_lane = bck.detect_white(img)

    height, width, c = img.shape
    points = bck.trackbar_values()

    # Warping step
    img_warp = bck.warp_img(img_lane, points, width, height)
    img_warp_points = bck.draw_points(img_copy, points)

    # Curve Detection Step
    mid_point, img_curve = bck.get_curve(
        img_warp, display=True, min_per=0.5, region=4)
    curve_avg_point, img_curve = bck.get_curve(
        img_warp, display=True, min_per=0.9)
